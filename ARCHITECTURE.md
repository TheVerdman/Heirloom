...
Python parity uses Python 3.13.12 and the committed `uv.lock`, which pins CPU PyTorch, NumPy, pytest, Ruff, and maturin. The checked-in fixture is stamped `generated_by: torch 2.12.0` and includes core tensor/autograd cases plus transformer-runtime cases for batched matmul, tanh-GELU, layer norm, embedding scatter-add, and fused causal attention.
...
`scripts/python_parity.sh` syncs `.venv-parity`, builds the extension, and runs `python/tests/test_heirloom_py_parity.py`. Those tests compare Heirloom directly against live PyTorch for matmul/ReLU/mean backward, broadcast plus transpose gradients, layer norm, embedding repeated-row accumulation, cross-entropy, fused causal attention, NumPy round-trips, and an explicitly skipped local-CUDA smoke when CUDA is unavailable.
