...
| `nvidia.nemotron_cc.high_actual` | conditionally approved for internal training from NVIDIA sample artifact | `nvidia/Nemotron-Pretraining-Dataset-sample` includes Nemotron-CC high-quality subsets under NVIDIA's Data Agreement for Model Training. This is not Apache-2.0 and does not approve raw public redistribution. |
...
| `nvidia.nemotron_cc_math` | conditionally approved for internal training from NVIDIA sample artifact | `nvidia/Nemotron-Pretraining-Dataset-sample` includes a Nemotron-CC-MATH subset under NVIDIA's Data Agreement for Model Training. Full Nemotron-CC-Math artifacts outside that sample still need exact license review. |
