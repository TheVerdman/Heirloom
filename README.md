...
- The memory transformer uses exact or product-key lookup, selected-row gradient paths, sparse-row AdamW, SMFT row masks, checkpoint family metadata, and CPU/CUDA-specific tests.
...
| Python/PyTorch parity | `./scripts/python_parity.sh` | Syncs `uv.lock`, builds the PyO3 extension, and compares against CPU PyTorch. The current local result is 7 passed and 1 CUDA-only test skipped. |
